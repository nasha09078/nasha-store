...
<template>
  <div class="flex  min-h-[90%] p-4 md:p-12 justify-center items-center">
    <div v-if="orders && orders.length" class="w-full">
      <table class="w-full text-left table-auto" aria-label="Order List">
        ...
        <tbody>
          <tr v-for="order in orders" :key="order.orderNumber" class="cursor-pointer hover:underline"
    ...
    <div v-else-if="orders && orders.length === 0"
      class="flex flex-col items-center justify-center flex-1 gap-4 min-h-[500px]">
      <n-result status="info" title="No orders found." >
        <template #footer>
          <n-button @click="router.push({ path: '/' });" type="primary">Back shopping</n-button>
        </template>
      </n-result>
    </div>
